...
export function Services({ onEnquiry }: { onEnquiry: () => void }) {
  return (
    <section className="py-32 md:py-40 relative overflow-hidden" style={{ background: "linear-gradient(180deg, #FFFFFF 0%, #F8F7FF 100%)" }}>
      ...
      <div className="container mx-auto px-6 md:px-12 mb-20 text-center">
        <div className="flex items-center justify-center gap-3 mb-6">
          ...
          <span className="text-sm font-semibold tracking-[0.2em] uppercase" style={{ color: "#7B5CF6" }}>What We Offer</span>
          <span className="w-12 h-[2px] rounded-full" style={{ background: "linear-gradient(90deg, #34D399, #7B5CF6)" }} />
        </div>
        <h2 className="text-4xl md:text-6xl lg:text-7xl font-heading font-bold tracking-tight mb-6" style={{ color: "#1D1D2B" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
          {services.map((service, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.05 }} whileHover={{ y: -12 }}
              className="group relative p-6 md:p-8 rounded-[30px] flex flex-col items-center text-center cursor-pointer transition-all duration-500"
              style={{ background: "#F8F7FF", border: "1px solid rgba(123,92,246,0.06)", boxShadow: "0 4px 20px rgba(123,92,246,0.04)" }}
            >
              <div className="absolute inset-0 rounded-[30px] opacity-0 group-hover:opacity-100 transition-all duration-700" style={{ boxShadow: "0 10px 40px rgba(123,92,246,0.12)" }} />
              <div className="w-16 h-16 rounded-[30px] flex items-center justify-center mb-5 transition-all duration-300 group-hover:scale-110" style={{ background: "rgba(123,92,246,0.06)", color: "#7B5CF6" }}>{service.icon}</div>
              <h3 className="font-bold text-sm mb-2 relative z-10" style={{ color: "#1D1D2B" }}>{service.name}</h3>
