...
export function Testimonials({ onEnquiry }: { onEnquiry: () => void }) {
  ...
  return (
    <section className="py-32 md:py-40 relative overflow-hidden" style={{ background: "#49225B" }}>
      <div className="absolute bottom-[10%] left-[5%] w-[300px] h-[300px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(227,170,221,0.15) 0%, transparent 70%)", filter: "blur(80px)" }} />
      ...
      <div className="container mx-auto px-6 md:px-12 mb-20 text-center">
        <div className="flex items-center justify-center gap-3 mb-6">
          ...
          <span className="text-sm font-semibold tracking-[0.2em] uppercase" style={{ color: "#6E3482" }}>Testimonials</span>
        ...
        <h2 className="text-4xl md:text-6xl lg:text-7xl font-heading font-bold tracking-tight mb-6 text-white">
          What Our Clients <span style={{ color: "#6E3482" }}>Say</span>
        </h2>
        <div className="flex items-center justify-center gap-2 mb-2">
          <Volume2 className="w-4 h-4" style={{ color: "#6E3482" }} />
          <p className="text-sm" style={{ color: "#6E3482" }}>Click any video to play with sound</p>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 max-w-6xl mx-auto">
          {videos.map((video, idx) => (
            <motion.div key={video.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: idx * 0.15 }}
              ...
              style={{ background: "#000", boxShadow: "0 20px 60px rgba(0,0,0,0.4)", border: activeVideo === video.id ? "2px solid #6E3482" : "1px solid rgba(255,255,255,0.1)" }}
            ...
            >
              ...
              {activeVideo === video.id && (
                <div className="absolute top-4 right-4 flex items-center gap-2 px-3 py-1.5 rounded-full pointer-events-none" style={{ background: "rgba(227,170,221,0.7)", backdropFilter: "blur(8px)" }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mt-16">
        <button onClick={onEnquiry} className="px-8 py-3 rounded-full text-sm uppercase tracking-widest inline-flex items-center gap-2 font-semibold transition-all duration-300 hover:bg-white/10" style={{ border: "1px solid rgba(196,181,253,0.3)", color: "#6E3482" }}>
      ...
      <div className="absolute bottom-0 left-0 right-0">
        <svg viewBox="0 0 1440 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full">
          <path d="M0 120L60 110C120 100 240 80 360 70C480 60 600 60 720 65C840 70 960 80 1080 85C1200 90 1320 90 1380 90L1440 90V120H1380C1320 120 1200 120 1080 120C960 120 840 120 720 120C600 120 480 120 360 120C240 120 120 120 60 120H0Z" fill="#F5EBFA"/>
        </svg>
      </div>
    </section>
  );
