    ...
    <section id="process" className="py-32 md:py-40 relative" style={{ background: "#F5EBFA" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {steps.map((step, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: index * 0.1 }} whileHover={{ y: -8 }}
              ...
              style={{
                background: index % 2 === 0 ? "#E7DBEF" : "#49225B",
                border: index % 2 === 0 ? "1px solid rgba(227,170,221,0.08)" : "none",
                boxShadow: index % 2 === 0 ? "0 4px 30px rgba(227,170,221,0.05)" : "0 4px 30px rgba(0,0,0,0.2)",
              }}
            >
              <div className="absolute -top-2 -right-1 text-[7rem] font-heading font-bold leading-none pointer-events-none" style={{ color: index % 2 === 0 ? "rgba(227,170,221,0.05)" : "rgba(255,255,255,0.05)" }}>{step.num}</div>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-6" style={{ background: index % 2 === 0 ? "rgba(227,170,221,0.08)" : "rgba(227,170,221,0.2)", color: index % 2 === 0 ? "#6E3482" : "#6E3482" }}>{step.icon}</div>
              <div className="text-xs uppercase tracking-[0.3em] font-semibold mb-3" style={{ color: index % 2 === 0 ? "#6E3482" : "#6E3482" }}>Step {step.num}</div>
              <h3 className="text-2xl font-bold mb-6 font-heading" style={{ color: index % 2 === 0 ? "#49225B" : "#E7DBEF" }}>{step.title}</h3>
              <ul className="space-y-3">
                {step.items.map((item, i) => (
                  <li key={i} className="flex items-center gap-3">
                    <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: "linear-gradient(135deg, #6E3482, #34D399)" }} />
                    <span className="text-sm" style={{ color: index % 2 === 0 ? "#4B5563" : "#6E3482" }}>{item}</span>
                  </li>
                ))}
              </ul>
      ...
      <div className="absolute bottom-0 left-0 right-0">
        <svg viewBox="0 0 1440 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full">
          <path d="M0 120L60 110C120 100 240 80 360 70C480 60 600 60 720 65C840 70 960 80 1080 85C1200 90 1320 90 1380 90L1440 90V120H1380C1320 120 1200 120 1080 120C960 120 840 120 720 120C600 120 480 120 360 120C240 120 120 120 60 120H0Z" fill="#49225B"/>
        </svg>
      </div>
    </section>
  );
